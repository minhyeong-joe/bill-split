...
export default function ItemizedResultScreen({route}) {
  ...
  const { items, members, memberItems, tipPercent, taxPercent, roundUp } = route.params.data;
  ...
  members.forEach(member => {
    ...
    let myItems = memberItems.filter(memberItem => memberItem.memId == member.id).map(memberItem => memberItem.itemId);
    ...
    myItems.forEach(itemId => {
      ...
      if (counts.filter(c => c.itemId == itemId).length > 0) {
        ...
        count = memberItems.reduce((acc, memberItem) => {
          return memberItem.itemId == itemId && memberItem.memId != member.id? acc + 1 : acc;
        }, 1);
      ...
      memberCharges.push({memId: member.id, name: member.name, itemId: item.id, itemName: item.item, itemWholePrice: item.price, itemPortionPrice: portionPrice});
    });
  });
  ...
  if (roundUp) {
    combinedTip = getTipRoundUp(combinedSubtotal, tipPercent, combinedTax, members.length);
  ...
  return (
    <View style={commonStyles.container}>
      ...
      {members.map(member => {
        ...
        <View key={member.id}>
          ...
          <View>
            ...
            <View style={{borderColor:'#999', borderTopWidth:1, width: '80%', alignSelf:'center', marginTop: 8}}></View>
            <View style={[styles.row, {marginBottom: 8}]}>
              <Text style={[styles.label, {fontSize: 18}]}>Total: </Text>
              <Text style={[styles.value, {fontSize: 18}]}>$ {total.toFixed(2)}</Text>
            </View>
          </View>
        </View>
      )})}
      ...
      <View>
        ...
        <View style={styles.row}>
          <Text style={styles.label}>Tax: </Text>
          <Text style={styles.value}>$ {combinedTax.toFixed(2)} ({taxPercent.toFixed(1)} %)</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.label}>Tip: </Text>
          <Text style={styles.value}>$ {combinedTip.toFixed(2)} ({tipPercent.toFixed(1)} %)</Text>
        </View>
        <View style={[styles.row, {marginBottom: 8, borderTopWidth: 1, borderColor: '#999', marginTop: 10}]}>
          <Text style={[styles.label, {fontSize: 18}]}>Total: </Text>
          <Text style={[styles.value, {fontSize: 18}]}>$ {combinedTotal.toFixed(2)}</Text>
        </View>
      </View>  
...
function getSubtotal(memberCharges, memId) {
  let subtotal = memberCharges.reduce((subtotal, memCharge) => {
    return memCharge.memId == memId? subtotal + memCharge.itemPortionPrice : subtotal
  }, 0);
  return subtotal;
}
